...
def get_auth(env, product, user="prescriber"):
    ...
    if product == "EPS-FHIR-DISPENSING-JWT":
        # In INT and REF, EPS-FHIR-DISPENSING-JWT uses the real /oauth2 endpoint
        # instead of the /oauth2-mock endpoint used by other products.
        # This preserves existing behavior for other products that already work with the mock.
        if env in ["int", "ref"]:
